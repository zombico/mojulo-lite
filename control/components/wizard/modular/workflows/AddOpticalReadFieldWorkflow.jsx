...
export default function AddOpticalReadFieldWorkflow({ onClose, onAddField, existingIdNames = [] }) {
  ...
  const handleLabelChange = (value) => {
    setFormData(prev => ({
      ...prev,
      label: value,
      idName: idTouched ? prev.idName : slugifyIdName(value),
    }));
    if (errors.label) {
      setErrors(prev => { const n = { ...prev }; delete n.label; return n; });
    }
    if (!idTouched && errors.idName) {
      setErrors(prev => { const n = { ...prev }; delete n.idName; return n; });
    }
  };

  const handleIdNameChange = (value) => {
    setIdTouched(true);
    setFormData(prev => ({ ...prev, idName: value }));
    if (errors.idName) {
      setErrors(prev => { const n = { ...prev }; delete n.idName; return n; });
    }
  };
  ...
  const handleHintChange = (value) => {
    setFormData(prev => ({ ...prev, hint: value }));
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg w-full max-w-lg mx-4 shadow-xl border border-gray-700 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
          <h3 className="text-lg font-medium text-gray-100">{t('addOpticalReadField')}</h3>
          <button
            type="button"
            onClick={onClose}
