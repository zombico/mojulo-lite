...
export async function POST(request) {
  try {
    ...
    // (notably triage routers) ships the keyword RAG cartridge generated by
    // docker.js from route descriptions.
    const ragMode = enabledProtocols.knowledge ? 'vector' : 'keyword';

    if (ragMode === 'vector' && !embeddings?.storageKey) {
      return NextResponse.json(
        { error: 'Knowledge protocol requires embeddings.storageKey from /api/vectorize-rag' },
        { status: 400 }
      );
    }
