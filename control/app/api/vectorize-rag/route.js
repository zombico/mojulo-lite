 ...
 *
 * Wizard-side embedding generation. Mirrors the chat builder's vector branch
 * in lib/builder/tool-executors.js (processDocumentsVector) but is keyed
 * by an opaque session token (botName + timestamp) instead of a builder
 * session id. Persists a single JSON blob at storage path
 *
 ...
 *
 * Body: { documents: [{ id, storagePath, originalName }], wizardToken? }
 * Returns: { storageKey, model, chunkCount, totalChunks, summary }
 */
...
import { chunkDocuments } from '@/lib/embedder/chunker';
...
export async function POST(request) {
  ...
  const { documents = [], wizardToken: existingToken } = body || {};
  ...
  if (parsed.length === 0) {
    return NextResponse.json(
      { error: 'No documents could be parsed for embedding' },
      { status: 400 }
    );
  }
  ...
  if (chunks.length === 0) {
    return NextResponse.json(
      { error: 'No chunks produced from documents' },
      { status: 400 }
    );
  }
